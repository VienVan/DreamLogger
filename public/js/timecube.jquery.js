        ...
        base.init = function(){
        	...
        	$(base.options.data).each(function(index, d){
				...
				if(base.options.showDate){
					...
					base.eventDivs[index].html("<h1>" + d.title + "</h1><p>" + d.description + "</p><button class='btn delete-btn fa fa-times' onclick='dreamCatcher.deleteDream(event)' id='" + d.id + "'>" + d.remove + "</button><button class='btn edit-btn fa fa-pencil' data-toggle='modal' data-target='#dreamModal' id='" + d.id + "'>" + d.edit + "</button>");
				}
